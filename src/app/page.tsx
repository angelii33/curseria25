      ...
      <div className="curso-cuerpo">
        ...
        {!curso.inscrito && curso.abiertaTitulo && (
          <p className="curso-abierta">
            <span className="curso-abierta-etiqueta">Prueba sin registro</span>
        ...
        <div className="curso-pie">
          ...
          <span className="curso-accion">
            ...
            <span className="t-interfaz" style={{ color: "var(--musgo-600)" }}>{cta}</span>
          </span>
        </div>
      </div>
    </Link>
...
export default async function Catalogo() {
  ...
  const conGratis = cursos.filter((c) => c.gratis > 0 && !c.inscrito);
  ...
  return (
    <>
      ...
      <main>
        <div className="marco">
          <header className="hero">
            ...
            <p className="t-lectura-guia" style={{ color: "var(--tinta-media)" }}>
              ...
              Cada curso arregla una cosa de tu negocio y termina con esa cosa hecha,
              no con apuntes que nunca vas a releer.
            </p>
            <div className="hero-acciones">
              <a className="btn btn-primario" href="#cursos">Ver cursos</a>
              {conGratis.length > 0 && (
                <a className="btn btn-secundario" href="#probar">
                  Probar una lección gratis
                </a>
              )}
              {!usuario && conGratis.length === 0 && (
                <Link className="btn btn-secundario" href="/entrar">Entrar</Link>
              )}
            </div>
          </header>
          ...
          {cursos.length > 0 && (
            <div className="franja">
              <p className="t-cuerpo">
                {cursos.length} cursos prácticos. Cada uno deja al menos una lección
                abierta para que compruebes la calidad sin registrarte.
              </p>
            </div>
          )}
        ...
        {conGratis.length > 0 && (
          <div className="marco" id="probar">
            <section className="superficie" style={{ marginTop: "var(--e-8)" }}>
              <div className="t-folio">Sin registro</div>
              <h2 className="t-titulo-2" style={{ marginTop: "var(--e-2)" }}>
                Prueba una lección antes de decidir
              </h2>
              <p className="t-cuerpo" style={{ marginTop: "var(--e-4)", color: "var(--tinta-media)", maxWidth: "58ch" }}>
                Cada curso abre al menos una lección completa. Ves el diseño,
                haces el ejercicio y decides si te sirve — sin crear cuenta.
              </p>
              <Perforacion sangrada />
              <ul className="entregables">
                {conGratis.slice(0, 4).map((c) => (
                  <li key={c.id}>
                    <Insignia tono="estado">{c.gratis} gratis</Insignia>
                    <span>
                      <Link href={`/cursos/${c.slug}`} className="t-interfaz" style={{ color: "var(--musgo-600)" }}>
                        {c.title}
                      </Link>
                      {c.abiertaTitulo && (
                        <span className="t-dato" style={{ display: "block", color: "var(--tinta-media)", marginTop: 2 }}>
                          Empieza por «{c.abiertaTitulo}»
                        </span>
                      )}
                    </span>
                  </li>
