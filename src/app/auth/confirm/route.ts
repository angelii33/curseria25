...
// Regreso de «Continuar con Google»: ?code=… se canjea por la sesión (PKCE;
// solo en el navegador que inició, donde está la galleta del verificador).
// El acceso con código por correo se quitó; ?token_hash=… se sigue
// aceptando solo por si Supabase manda algún enlace de confirmación de
// cuenta (cuando «Confirm email» está encendido y no hay llave de servicio).
// La sesión queda en galletas del servidor, como el resto.

const TIPOS: EmailOtpType[] = ["email", "magiclink", "signup", "invite", "recovery", "email_change"];

export async function GET(request: NextRequest) {
  const p = request.nextUrl.searchParams;
  const next = rutaInterna(p.get("next"), "/mi-aprendizaje");
